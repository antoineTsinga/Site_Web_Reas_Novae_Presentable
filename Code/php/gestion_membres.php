...
if(isset($_POST['search-member']) AND !empty($_POST['search-member'])) 
{	
	...
	if (isset($_POST['civilite'])) 
	{
		...
		$serialNames2 = $db->prepare('SELECT Nom,Prenom,Email,Telephone,idUtilisateur,Sexe FROM utilisateur WHERE Sexe LIKE "%'.$Sexe.'%" AND (Nom LIKE "%'.$q.'%" OR Prenom LIKE "%'.$q.'%") ORDER BY idUtilisateur');
		...
		$serialNamesGestion = $db->prepare('SELECT Nom,Prenom,Email,Telephone,idGestionnaire,Sexe FROM gestionnaire WHERE Sexe LIKE "%'.$Sexe.'%" AND (Nom LIKE "%'.$q2.'%" OR Prenom LIKE "%'.$q2.'%") ORDER BY idGestionnaire');
		$serialNamesGestion->execute();
		$afficheNamesGestion = $serialNamesGestion->fetchAll();

									//Administrateurs
		$q3 = htmlspecialchars($_POST['search-member']);
		$serialNamesAdmin = $db->prepare('SELECT Nom,Prenom,Email,Telephone,idAdministrateur,Sexe FROM administrateur WHERE Sexe LIKE "%'.$Sexe.'%" AND (Nom LIKE "%'.$q3.'%" OR Prenom LIKE "%'.$q3.'%") ORDER BY idAdministrateur');
