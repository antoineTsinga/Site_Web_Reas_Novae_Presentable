...
CREATE TABLE IF NOT EXISTS `mydb`.`Appareil de Test` (
  ...
  `Numero_de_version` VARCHAR(45) NULL,
...
CREATE TABLE IF NOT EXISTS `mydb`.`Utilisateur` (
  ...
  `Gestionnaire_idGestionnaire` INT,
  PRIMARY KEY (`idUtilisateur`),
  INDEX `fk_Utilisateur_Gestionnaire1_idx` (`Gestionnaire_idGestionnaire` ASC),
  CONSTRAINT `fk_Utilisateur_Gestionnaire1`
    FOREIGN KEY (`Gestionnaire_idGestionnaire`)
    REFERENCES `mydb`.`Gestionnaire` (`idGestionnaire`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `mydb`.`Resultat` (
  ...
  INDEX `fk_Resultat_test1_idx` (`test_idtest` ASC),
